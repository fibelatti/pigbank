...
class AddGoalPresenter(
    ...
    override fun createGoal(description: String, cost: String, deadline: String) {
        validateGoalUseCase.validateGoal(now = Date(), goalCandidate = goalDomainMapper.toDomainModel(description, cost, deadline))
            ...
            .subscribeUntilDetached(
                {
                    ...
                    view?.onGoalCreated(goal = goalPresentationMapper.toPresentationModel(it))
                },
                { throwable ->
                    when (throwable) {
